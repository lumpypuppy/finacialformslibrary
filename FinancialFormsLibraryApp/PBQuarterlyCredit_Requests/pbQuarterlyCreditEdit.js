...
$('input[title="Percent"]').attr("disabled", true);
$('input[title="Target Amount"]').attr("disabled", true);

var value = document.querySelectorAll('input[value="Save"]')[1].id;
console.log(value);
document.querySelector("#"+value).value = 'Submit';
document.querySelector("[title='Year Required Field']").style.width = "100px";
}); 
</script>
...
#part1 > table.ms-formtable > tbody > tr:nth-child(5) {
...
#part1 > table > tbody > tr:nth-child(3) > td > table > tbody > tr:nth-child(5) {
    display: grid;
}
